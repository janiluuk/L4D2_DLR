...
public void OnLibraryAdded(const char[] name)
{
	if (strcmp(name, "extra_menu") == 0)
	{
		...
		ExtraMenu_AddEntry(menu_id, "7. Vote for custom gamemode", MENU_SELECT_LIST);
		...
		ExtraMenu_AddEntry(menu_id, "1. Halt game: _OPT_", MENU_SELECT_LIST);
		...
		ExtraMenu_AddEntry(menu_id, "3. Debug mode: _OPT_", MENU_SELECT_LIST);
		ExtraMenu_AddOptions(menu_id, "Off|Log to file|Log to chat|Tracelog to chat");
		ExtraMenu_AddEntry(menu_id, "4. Infected spawn: _OPT_", MENU_SELECT_ONOFF, false, 1);

		ExtraMenu_AddEntry(menu_id, " ", MENU_ENTRY);
		ExtraMenu_AddEntry(menu_id, "DEBUG COMMANDS:", MENU_ENTRY);
		ExtraMenu_AddEntry(menu_id, "1. Reload", MENU_SELECT_LIST);
		ExtraMenu_AddOptions(menu_id, "Map|DLR Plugins|All plugins|Restart server");

		ExtraMenu_AddEntry(menu_id, "2. Infected spawn: _OPT_", MENU_SELECT_ONOFF, false, 1);
		ExtraMenu_AddEntry(menu_id, "3. Remove weapons from map", MENU_SELECT_ONLY);
		ExtraMenu_AddEntry(menu_id, "4. Manage skills", MENU_SELECT_ONLY, true);
		ExtraMenu_AddEntry(menu_id, "5. Manage perks", MENU_SELECT_ONLY, true);
